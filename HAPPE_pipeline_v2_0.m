...
task_segment_start = -0.3;
task_segment_end = 0.7;
...
%% make output folders:
src_folder_name = outputFolder;
if ~exist(src_folder_name, 'dir')
    mkdir(src_folder_name)
end
...
% cd get file list
cd (src_folder_name);
[~,~,src_file_ext] = fileparts(FileNames{1});
...
for current_file = 1:length(FileNames)
    ...
    if task_EEG_processing == 2
        EEGloaded = pop_loadset(FileNames{current_file});
    ...
    try 
        if pipeline_visualizations_semiautomated == 0
            [wIC, A, W, IC] = wICA(EEG,icaType, 1, 0, [], 5);
        elseif pipeline_visualizations_semiautomated == 1
            [wIC, A, W, IC] = wICA(EEG,icaType, 1, 1, srate, 5);
        end
    catch wica_err
    ...
    if strcmpi(icaType, 'runica')
        EEG = pop_runica(EEG, 'extended',1,'interupt','on', 'icatype', icaType);
    else
    ...
    EEG = pop_saveset(EEG, 'filename',strrep(FileNames{current_file}, src_file_ext,'_ICA.set'));
    ...
    EEG = pop_saveset(EEG, 'filename',strrep(FileNames{current_file}, src_file_ext,'_ICAcleanedwithMARA.set'));
    ...
    EEG = pop_saveset( EEG, 'filename',strrep(FileNames{current_file},src_file_ext,'_segmented.set'));
    ...
    if segment_interpolation
        ...
        EEG = pop_saveset(EEG, 'filename',strrep(FileNames{current_file}, src_file_ext,'_segments_interp.set'));
    end
    ...
    EEG = eeg_checkset(EEG );
    EEG = pop_saveset(EEG, 'filename',strrep(FileNames{current_file}, src_file_ext,'_segments_postreject.set'));
    ...
    switch save_as_format
        ...
        case 3 % .set file
            EEG = pop_saveset(EEG, 'filename',strrep(FileNames{current_file}, src_file_ext,'_processed.set'));
    end
